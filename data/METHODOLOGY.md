...
`culvers.js` is generated by `scripts/build_culvers.py` from the Culver's
restaurant locator (`culvers.com/api/locator/getLocations`), whose records carry
each restaurant's `openDate`, `restaurantNumber`, address, and coordinates. The
locator returns at most 100 restaurants within 600 km per query, so the script
covers the continental US with an adaptive quadtree (a full cell splits in four);
the 2026-09-18 crawl took 108 queries and found 1,125 restaurants.

- **Dates are Culver's own**, stored to the month (`datePrecision: "month"`);
  the exact day is in each popup's note. Restaurant numbers run in opening order.
- **`announced`** = listed in the locator with an opening date after the build
  date (41 on 2026-09-18, through March 2027). Sites reported only in the news
  (e.g. first stores in Virginia and New York) are not included.
- **No speculative tier.** At 50+ openings a year a site-by-site projection
  would be mostly noise.
- **Survivorship.** The locator lists only restaurants operating today, so closed
  restaurants are absent and historical counts can run slightly low. Culver's
  closes very few (one or two a year in its 2026 franchise disclosure document).
  Coordinates are Culver's own (`geoPrecision: "store"`).

### Cross-checks (enforced by the build script)

| Anchor | Dataset | Independent source |
|---|---|---|
| #1 | Sauk City WI, July 1984 | Wikipedia |
| #300 | Fond du Lac WI, Oct 3, 2005 | QSR Magazine, "Culver's Hits 300 Units" |
| #500 | Cedar Falls IA, Mar 3, 2014 | Culver's press release |
| #1000 | Fort Wayne (New Haven) IN, Dec 2024 | Fort Wayne local news |
| Open at end of 2022 / 2023 / 2024 / 2025 | 890 / 943 / 996 / 1,041 | 892 / 944 / 997 / 1,041 (2026 FDD, Item 20) |
| First restaurant outside Wisconsin | Buffalo MN, Sept 1995 | Wikipedia |
| States | 26 | 26 (2026 FDD) |
